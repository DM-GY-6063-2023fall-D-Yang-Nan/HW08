...
function preload() {
  origImg = loadImage('picture.jpg');  // 从项目目录加载 Mondrian 图像
  replacementImg = loadImage('111.jpg');  // 加载要替换的图像
}

function setup() {
  createCanvas(windowWidth, windowHeight);
  
  origImg.resize(0, height);  // 调整原始图像的大小以匹配浏览器窗口高度
  img = createImage(origImg.width, origImg.height);  // 创建一个新图像对象来存储原始图像的副本
  
  replacementImg.resize(origImg.width, origImg.height);  // 调整替换图像的大小以匹配原始图像的大小
  
  cp = createColorPicker("#ff0000");  // 创建一个初始颜色为红色的颜色选择器
  cp.position(10, 10);  // 设置颜色选择器的位置
